...
def prepare_vao_frame_white():
    # prepare vertex data (in main memory)
    vertices = glm.array(glm.float32,
        # position        # color
         -5.0, 0.0, 0.0,  1.0, 1.0, 1.0, # x-axis start
         5.0, 0.0, 0.0,  1.0, 1.0, 1.0, # x-axis end 
         0.0, 0.0, 0.0,  1.0, 1.0, 1.0, # y-axis start
         0.0, 0.0, 0.0,  1.0, 1.0, 1.0, # y-axis end 
         0.0, 0.0, 0.0,  1.0, 1.0, 1.0, # z-axis start
...
def main():
    ...
    M_loc_vertical = glGetUniformLocation(shader_program, 'M_v')
    
    # prepare vaos: 2개 그릴거라 2개 준비함(본래는 하나에 그려도 되긴한데 성능 고려할게 아니라서 이렇게 함)
    vao_triangle_horizontal = prepare_vao_triangle()
    vao_triangle_vertical = prepare_vao_triangle()
    vao_frame = prepare_vao_frame()
    vao_line = prepare_vao_frame_white()
    ...
    while not glfwWindowShouldClose(window):
        ...
        # draw triangle
        glBindVertexArray(vao_triangle_horizontal)
        glDrawArrays(GL_TRIANGLES, 0, 3)


        glUniformMatrix3fv(M_loc, 1, GL_TRUE, T_v)

        glBindVertexArray(vao_triangle_vertical)
        glDrawArrays(GL_TRIANGLES, 0, 3)
